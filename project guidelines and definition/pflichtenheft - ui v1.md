...
6. Master Workflow
 ...
 Systemablauf (End-to-End):

1. Bestellung starten
2. Daten laden (Server → IndexedDB)
3. Offline-Rundgang
4. Lokale Speicherung
5. Rückkehr online
6. Kontrollansicht
7. Lieferantenzuordnung (final)
8. Ausgabe (Mail / Liste)  

7. Kritische Stellen

Wenn Artikel mehrere Lieferanten hat:
→ nur Lieferanten berücksichtigen, die liefern können

ja, aber hier prüfen wir nur den avisierten Liefertag

→ nach Priorität wählen

wenn mehrere Lieferanten den Artikel am selben Tag liefern können gerne

→ Fallback?

Händische kontrolle aller fragwürdigen Artikel (vor Versendung der Bestellungen zeige eine Liste von Artikeln, die mehrere Lieferanten haben und zeige was die automatische wahl ist, aber lass diese Manuell änderbar)